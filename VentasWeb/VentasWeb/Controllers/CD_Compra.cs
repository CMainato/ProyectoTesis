...
namespace CapaDatos
{
    public class CD_Compra
    {
                    ...
                    using (XmlReader dr = cmd.ExecuteXmlReader())
                    {
                        while (dr.Read())
                        {
                            ...
                            if (doc.Element("DETALLE_COMPRA") != null)
                            {
                                ...
                                rptDetalleCompra.oAgencia = (from dato in doc.Element("DETALLE_COMPRA").Elements("DETALLE_AGENCIA")
                                                            select new Agencia()
                                                            {
                                ...
                                rptDetalleCompra.oListaDetalleCompra = (from producto in doc.Element("DETALLE_COMPRA").Element("DETALLE_PRODUCTO").Elements("PRODUCTO")
                                                                        select new DetalleCompra()
                                                                        {
                                                                            ...
                                                                            NumeroFactura = producto.Element("NumeroFactura").Value
        ...
        public List<Compra> ObtenerListaCompra(DateTime FechaInicio, DateTime FechaFin, int IdProveedor, int IdAgencia)
        {
                ...
                try
                {
                    ...
                    while (dr.Read())
                    {
                        rptListaCompra.Add(new Compra()
                        {
                            ...
                            oAgencia = new Agencia() { Nombre = dr["Nombre"].ToString() },
